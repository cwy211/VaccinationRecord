...
import controller.FullVaxByState;

public class DisplayFullVaxByState {
	...
	private void initialize() {
		...
		frmFullyVaccinatedPeople.setTitle("Fully Vaccinated People By State");
        ...
        List<VaxState> dataList=VaxState.getVaxStateList();
		List<VaxState> result = FullVaxByState.getResult(dataList);
		result.stream().forEach(elem->{
			Vector<String> row=new Vector<String>();
	        row.add(elem.getState());
	    	row.add(elem.getCumul_full());
	    	csv_data.addRow(row);
		});
		...
		lblNewLabel = new JLabel("Fully Vaccinated People By State");
